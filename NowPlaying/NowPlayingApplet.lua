...
function _getXtraMetaData(self)
	...
	log:debug('enabledSkinName = '..tostring(enabledSkinName))
	...
	if (server and enabledSkinName == 'QVGAlandscapeSkin') then
...
function init(self)
	...
	local settingsItems = {"displayAudioMetaData", "displayStatusIcons", "displayRating", "displayYear"}
...
function _setTitleStatus(self, text, duration)
	...
	self.statuslossless:setStyle("islossless_1")
...
function _createUI(self)
	...
	if not self:getSettings()["displayAudioMetaData"] and self:getSettings()["displayYear"] then
		self.npaudiometaGroup = Group('npaudiometagroupyearonly', {
			npaudiometa = self.mytrackaudiometa,
		})
		self.npstatusiconGroupNorth = Group('npstatusicongroupnorthyearonly', {
			npstatusremote = self.statusremote,
			npstatuscsst = self.statuscsst,
		})
		self.npstatusiconGroupSouth = Group('npstatusicongroupsouthyearonly', {
		...
		self.npstatusiconGroupNorth = Group('npstatusicongroupnorth', {
			npstatusremote = self.statusremote,
			npstatuscsst = self.statuscsst,
		})
		self.npstatusiconGroupSouth = Group('npstatusicongroupsouth', {
			npstatuslossless = self.statuslossless,
			npstatuslyrics = self.statuslyrics,
		})
	end
	...
	if self:getSettings()["displayStatusIcons"] then
		window:addWidget(self.npstatusiconGroupNorth)
		window:addWidget(self.npstatusiconGroupSouth)
	end
